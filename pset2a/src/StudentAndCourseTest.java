...
//		int newCredits = legacyChild.getCredits(); //(a > b ? b : a)
    ...
    public void testInstructorInit() {
        Instructor i = new Instructor("Victor", "Li Wang", "RebootU");
        assertEquals("Victor Li Wang", i.getName());
        assertEquals("RebootU", i.getSchool());
    }

    public void testPersonInit() {
        Person p = new Person("Victor", "Li Wang");
        assertEquals("Victor Li Wang", p.getName());
    }

    public void testFullTimeStudentInit() {
        Student fts = new FullTimeStudent("Doug", "Shook", 111111);
        fts.setCredits(35);
        assertEquals(35, fts.getCredits());
        assertEquals(47000.0, fts.computeTuition());
        //assertEquals("Doug Shook", s.getName());
        //assertEquals(111111, s.getStudentID());
        //No credits, no GPA
        //assertEquals(0.0, s.getGPA());
